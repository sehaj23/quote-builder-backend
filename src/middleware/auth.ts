...
export const authenticateToken = async (req: Request, res: Response, next: NextFunction): Promise<Response | void> => {
  ...
  try {
    ...
    // Verify user exists in database (like EventNest does)
...
// Role-based access control (following EventNest pattern)
export const requireApproved = (req: Request, res: Response, next: NextFunction): Response | void => {
  // @ts-ignore
  if (!req.user) {
    return res.status(401).json({
      success: false,
...
export const requireSuperUser = (req: Request, res: Response, next: NextFunction): Response | void => {
  // @ts-ignore
  if (!req.user) {
    return res.status(401).json({
      success: false,
