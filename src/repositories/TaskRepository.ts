...
export class TaskRepository {
  ...
  async create(taskData: CreateTaskRequest): Promise<number> {
    try {
  ...
  async findByQuoteId(quoteId: number, filters: TaskFilterOptions = {}): Promise<Task[]> {
    try {
      const connection = this.getDbConnection();
  ...
  async update(taskId: number, taskData: UpdateTaskRequest): Promise<boolean> {
    try {
      if (!taskId) {
      ...
      const updatableFields: (keyof UpdateTaskRequest)[] = [
        'title',
        'description',
        'status',
        'priority',
  ...
  async getProgressSummary(quoteId: number): Promise<TaskProgressSummary> {
    try {
      const connection = this.getDbConnection();
      const [rows] = await connection.execute<RowDataPacket[]>(
        `SELECT
  ...
  async findPendingReminders(channel: 'whatsapp' | 'email' = 'whatsapp', limit = 100): Promise<Task[]> {
    try {
      const connection = this.getDbConnection();
      const [rows] = await connection.execute<RowDataPacket[]>(
        `SELECT *
    ...
    try {
      ...
      const [result] = await connection.execute<ResultSetHeader>(
        `UPDATE tasks
         ...
         WHERE id = ?`,
        [status, nextReminderAt, errorMessage || null, taskId]
      );
      return result.affectedRows > 0;
    } catch (error) {
